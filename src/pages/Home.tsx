...
import { getDateStr, formatSlot, formatSlotShort, getTodayStr, getTomorrowStr, getNegativeRanges } from "../utils/slotUtils";
import NegativePriceBanner from "../components/NegativePriceBanner";
import { Slide, ToastContainer, toast } from 'react-toastify';
import RateSummaryBox, { RateSummaryType } from "../components/RateSummaryBox";
import { RATES_PUBLISH_HOUR, TOAST_DURATION_MS } from "../constants/appConfig";
...
function Home() {
  ...
  const handleRefresh = useCallback(() => {
    ...
    const now = new Date();
    const isBefore4pm = now.getHours() < RATES_PUBLISH_HOUR;

    if (hasTodayRates && isBefore4pm) {
      toast(`Rates are up to date. New rates available after ${RATES_PUBLISH_HOUR}:00.`);
      return;
    }

    if (hasTodayRates && !isBefore4pm && hasTomorrowRates) {
      toast("Rates are up to date.");
      return;
    }
  ...
  return (
    <div>
      ...
      <NegativePriceBanner negativeLines={negativeLines} />
          ...
          {filteredRates.length === 0 ? (
            <div className="rounded-lg border border-slate-700 bg-slate-800 p-6 text-center">
              <p className="text-slate-400">
                ...
                published after {RATES_PUBLISH_HOUR}:00.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="mb-4 grid grid-cols-2 gap-4">
                <RateSummaryBox rate={lowest} type={RateSummaryType.Lowest} />
                <RateSummaryBox rate={highest} type={RateSummaryType.Highest} />
              </div>
